        ...
        try {
            ...
            form.parse(req)
                ...
                .on('error', function (err) {
                    ...
                    let dir_name = config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + fields.dir_name;
        ...
        try {
            ...
            form.parse(req)
                ...
                .on('error', function (err) {
                    ...
                    newOpenFile = this.openedFiles;
                    let dir_name = config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + fields.dir_name;
                    let thumb_dir_name = config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + fields.thumb_dir_name;
            ...
            form.parse(req)
                ...
                .on('error', function (err) {
                    ...
                    if (notFonud == 1) {
                        ...
                        dirKeyName = config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + "company/" + folder_name + "/profile/" + file_name;
                        var fileBody = fs.readFileSync(temp_path);
                        params = { Bucket: LowerCase_bucket, Key: dirKeyName, Body: fileBody, ACL: 'public-read-write' };
...
function fileUploadOnly(fileBody, LowerCase_bucket, folder_name, file_name, file_name_ext) {
    return new Promise(async function (resolve, reject) {
        ...
        dirKeyName = config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + "company/" + folder_name + "/" + file_name;
        file_name_ext = file_name_ext.toLowerCase()
            ...
            form.parse(req)
                ...
                .on('error', function (err) {
                    ...
                    if (notFonud == 1) {
                        var temp_path = this.openedFiles[0].path;
                        var file_name = this.openedFiles[0].name;
                        dirKeyName = config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + fields.dirkeyname + file_name;
                        var fileBody = fs.readFileSync(temp_path);
                        params = { Bucket: LowerCase_bucket, Key: dirKeyName, Body: fileBody, ACL: 'public-read-write' };
                        var urlProfile = config.wasabisys_url + "/" + LowerCase_bucket + "/" + config.SUPPLIER_DIVERSITY_WASABI_PATH + "/" + fields.dirkeyname + file_name;
                        console.log("urlProfile", urlProfile);
                        bucketOpration.uploadFile(params, async function (err, result) {
                            if (err) {
                                res.send({ message: translator.getStr('SomethingWrong'), status: false, data: "" });
