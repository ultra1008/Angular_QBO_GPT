...
@Component({
  selector: 'app-employee-payrollgroup',
  templateUrl: './employee-payrollgroup.component.html',
  styleUrls: ['./employee-payrollgroup.component.scss']
})
export class EmployeePayrollgroupComponent implements OnInit {
  ...
  getDataPayRollGroup() {
    let that = this;
    this.httpCall.httpGetCall(httproutes.PORTAL_SETTING_PAYROLLGROUP_ALL).subscribe(function (params) {
      if (params.status) {
  ...
  deletePayRollGroup(payroll: any) {
    ...
    swalWithBootstrapButtons.fire({
      title: this.Employee_Payrollgroup_Do_Want_Delete,
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        that.httpCall.httpPostCall(httproutes.PORTAL_SETTING_PAYROLLGROUP_DELETE, { _id: payroll._id }).subscribe(function (params) {
          if (params.status) {
            that.snackbarservice.openSnackBar(params.message, "success");
            that.getDataPayRollGroup();
          } else {
  ...
  addPayrollGroup(reqData: any) {
    const dialogRef = this.dialog.open(EmployeePayRollGroupForm, {
      data: reqData,
      disableClose: true
    });
...
@Component({
  selector: 'employee-payrollgroup-from',
  templateUrl: './employee-payrollgroup-from.component.html',
  styleUrls: ['./employee-payrollgroup.component.scss']
})
export class EmployeePayRollGroupForm implements OnInit {
  public payrollgroup: FormGroup;
  ...
  constructor(private modeService: ModeDetectService, public dialogRef: MatDialogRef<EmployeePayRollGroupForm>, public translate: TranslateService,
    @Inject(MAT_DIALOG_DATA) public data: any, public httpCall: HttpCall, public snackbarservice: Snackbarservice) {
    this.payrollgroup = new FormGroup({
      payroll_group_name: new FormControl("", [Validators.required]),
    });
    if (this.data) {
      this.payrollgroup = new FormGroup({
        payroll_group_name: new FormControl(this.data.payroll_group_name, [Validators.required]),
      });
    }
  ...
  saveData() {
    let that = this;
    if (this.payrollgroup.valid) {
      let reqData = this.payrollgroup.value;
      if (this.data) {
        reqData._id = this.data._id;
      }
      this.httpCall.httpPostCall(httproutes.PORTAL_SETTING_PAYROLLGROUP_SAVE, reqData).subscribe(function (params) {
        if (params.status) {
          that.snackbarservice.openSnackBar(params.message, "success");
