...
export class DocumentsService extends UnsubscribeOnDestroyAdapter {
  ...
  async getDocumentTable(is_delete: number, tab_Array: string): Promise<void> {
    const data = await this.httpCall.httpPostCall(httpversion.PORTAL_V1 + httproutes.DOCUMENTS_DATATABLE, { is_delete: is_delete, document_type: tab_Array }).toPromise();
